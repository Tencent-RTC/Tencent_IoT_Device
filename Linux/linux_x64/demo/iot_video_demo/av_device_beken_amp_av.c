...
//
// BK7258 AMP + AV 开发板：media_app UVC（MJPEG 连接 + MJPEG→H264 pipeline）+ aud_intf
// （PA GPIO_5）。
//
// 与 av_device_beken_amp_ai.c（AI/DVP）的区别：相机走 UVC_CAMERA；
// 分辨率见 iot_demo_config.h（AMP+AV → 480x320）；
// PA 由编译宏 AUD_DAC_PA_CTRL_GPIO=GPIO_5 覆盖（见 gpio_map.h ifndef）。
//
// UVC 连接必须用 IMAGE_MJPEG：常见模组（如 0c45:64ab）只枚举 MJPEG，若用
// IMAGE_H264 会等 USB_UVC_H26X_DEVICE connect 超时（media_app_camera_open=10）。
// H264 由 pipeline_h264 + pipeline_jdec 产出，对齐 AVDK doorbell / agora UVC 路径。
//
...
static camera_type_t s_camera_type = UVC_CAMERA;
static bool s_uvc_pipeline_opened = false;
...
int av_device_start_video(const char *video_file_path, video_device_callback_t callback,
  ...
  // UVC：先 MJPEG 连接（USB_UVC_DEVICE），再开 MJPEG→H264 pipeline 取编码帧。
  media_camera_device_t device = {0};
  device.type = UVC_CAMERA;
  // 与 AVDK doorbell / agora 一致：UVC 走 Hub Port 1。
  device.port = 1;
  ...
  if (ret != BK_OK) {
    LOGE("media_app_pipeline_h264_open failed: %d\n", (int)ret);
    goto fail_camera;
  }

  ret = media_app_pipeline_jdec_open();
  if (ret != BK_OK) {
    LOGE("media_app_pipeline_jdec_open failed: %d\n", (int)ret);
    media_app_pipeline_h264_close();
    goto fail_camera;
  }
  s_uvc_pipeline_opened = true;

  // 注册编码帧读取回调：取 pipeline 产出的 H264。
  ...
  HAL_Printf("[av_avdk][VID] video capture started: %ux%u@%dfps (UVC MJPEG->H264)\n", VIDEO_WIDTH,
...
int av_device_start_audio(const char *audio_file_path, audio_device_callback_t callback,
  ...
  // tc_iot/AMP AV: 默认 spk_gain=0x2d(71%) + 本板 PA GPIO_5 → 喇叭过响，被 mic 拾取形成正反馈啸叫，
...
// ==================== 事件抓拍图（云存图片）====================
// AV/UVC 路径暂无稳定 MJPEG 抓拍 API（与 AI/DVP mailbox 竞态同类问题），显式失败。
int av_device_get_event_picture(uint32_t event_id, uint8_t *buf, uint32_t buf_size,
  ...
  LOGE("get_event_picture: not supported on AMP AV/UVC path yet\n");
  return -1;
}
